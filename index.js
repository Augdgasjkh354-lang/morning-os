...
app.post('/api/chat/summarize', requireAuth, async (req, res) => {
  ...
  const settings = await readSettings(); const summarizePrompt = "你是记忆整理助手。分析以下对话，提炼值得长期记住的信息。严格只返回JSON，不输出任何其他内容：{\"high_confidence\":[],\"medium_confidence\":[],\"low_confidence\":[]}";
  let raw = '{"high_confidence":[],"medium_confidence":[],"low_confidence":[]}'; try { raw = await callDeepSeek(settings, [{ role: 'system', content: summarizePrompt }, { role: 'user', content: JSON.stringify(conv.messages || []) }]); } catch (e) { console.log('记忆提炼失败：', e.message); }
  let parsed = { high_confidence: [], medium_confidence: [], low_confidence: [] }; try { parsed = JSON.parse(raw); } catch {}
  const nowIso = new Date().toISOString(); const added = []; const merged = []; const conflicts = []; const skipped = (parsed.low_confidence || []).length;
  await writeMemoryLatest(MEMORY_PATH, readJson, writeJson, TIMEZONE, async (m) => { const toSave = [...(parsed.high_confidence || []).map((x) => ({ ...x, needs_review: false })), ...(parsed.medium_confidence || []).map((x) => ({ ...x, needs_review: true }))]; for (const item of toSave) { const t = ['identity', 'knowledge', 'inference'].includes(item.type) ? item.type : 'knowledge'; const found = (m[t] || []).find((x) => similarity(x, item) > 0.75); if (found) { found.content = item.content || found.content; found.tags = Array.from(new Set([...(found.tags || []), ...((item.tags) || [])])); found.date_updated = nowIso; merged.push(found); continue; } const created = { id: `mem_${randomUUID()}`, type: t, title: item.title || '未命名', content: item.content || '', tags: item.tags || [], source_type: item.source_type || 'conversation', source_conversation_id: conversation_id, source_message_ids: [], confidence: Number(item.confidence || 0), stability: item.stability || 'medium', needs_review: !!item.needs_review, importance: 8, decay_score: 10, use_count: 0, last_used_at: null, expires_at: ((item.source_type === 'news' || item.source_type === 'api') && t === 'knowledge') ? new Date(Date.now() + 7 * 86400000).toISOString() : null, conflict: false, history: [], date_created: nowIso, date_updated: nowIso }; (m[t] || []).push(created); added.push(created); } recalcTokens(m); return m; });
  conv.memory_summary_pending = false; conv.memory_summary_count = (conv.memory_summary_count || 0) + 1; conv.last_summarized_at = nowIso; conv.memory_summary = { added, merged, conflicts };
  await writeJson(cpath, conv);
  res.json({ status: 'done', added, merged, conflicts, skipped_low_confidence: skipped });
});
app.get('/api/chat/summarize', requireAuth, async (req, res) => res.json({ success: true }));

ensureBaseFiles().then(() => { startScheduler({ timezone: TIMEZONE, runDailyJob: () => runDailyJob({ readSettings, fetchAllData, writeJson, latestDataPath: LATEST_DATA_PATH, conversationsDir: CONVERSATIONS_DIR, reportsDir: REPORTS_DIR, dateStr: () => dateStr(TIMEZONE), CONSTITUTION }), decayTask: async () => { console.log('开始执行记忆衰减任务'); await runMemoryDecay(MEMORY_PATH, readJson, writeJson); }, summarizePendingTask: async () => { const files = (await fs.readdir(CONVERSATIONS_DIR).catch(() => [])).filter((f) => f.endsWith('.json')); for (const f of files) { const c = await readJson(path.join(CONVERSATIONS_DIR, f), {}); if (!c.memory_summary_pending) continue; console.log('开始处理待提炼对话：', c.id || f); } } }); app.listen(PORT, () => console.log(`服务已启动，端口：${PORT}`)); });
