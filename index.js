...
const { MEMORY_TEMPLATE, estimateTokens, kw, similarity, dateStr, recalcTokens, readMemory, writeMemoryLatest } = require('./lib/memoryManager');
...
app.get('/api/latest-data', async (req, res) => { try { res.json(await readJson(LATEST_DATA_PATH, {})); } catch { res.json({ empty: true }); } });
...
app.post('/api/memory/prune', requireAuth, async (req, res) => res.json({ suggestions: [] }));
