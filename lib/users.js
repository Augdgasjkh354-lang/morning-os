...
async function initSystem(){
  ...
  if(!users.length){
    const admin = {id:'user_admin',username:'999999',password_hash:await bcrypt.hash('999999111111',10),role:'admin',invite_quota:999999,invites_used:0,invited_by:null,created_at:new Date().toISOString(),last_login_at:'',is_active:true};
...
const genUserId=()=>`user_${nanoid(8)}`; const genInviteId=()=>`inv_${nanoid(8)}`;
