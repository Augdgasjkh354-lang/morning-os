...
async function runMemoryDecay(memoryPath, readJson, writeJson) {
  const now = new Date();
  const memory = await readJson(memoryPath, MEMORY_TEMPLATE);
  for (const type of ['knowledge', 'inference']) {
    const bucket = memory[type] || [];
    const kept = [];
    for (const item of bucket) {
      const updated = new Date(item.date_updated || item.date_created || now.toISOString());
      const days = Math.max(0, Math.floor((now - updated) / (1000 * 60 * 60 * 24)));
      const step = type === 'knowledge' ? 7 : 14;
      const drop = Math.floor(days / step);
      const nextScore = (item.decay_score ?? 10) - drop;
      const nextItem = { ...item, decay_score: nextScore, type: item.type || type };
      if (nextScore <= 0) {
        memory.archive = memory.archive || [];
        memory.archive.push(nextItem);
      } else {
        kept.push(nextItem);
      }
    }
...
module.exports = { MEMORY_TEMPLATE, estimateTokens, kw, similarity, dateStr, recalcTokens, readMemory, writeMemoryLatest, runMemoryDecay };
